    ...
    public class DefaultController : Controller
    {
        ...
        public ActionResult Cadastro(Users user)
        {
            try
            {
                ...
                {
                    Regex rgx = new Regex(@"(?!^[0-9]*$)(?!^[A-zA-Z]*$)^([a-zA-Z0-9]{8,15})$");
                    if (!rgx.IsMatch(user.Password))
                    {
                        ModelState.AddModelError("Password", "A senha deve ser alfanumérica com pelo menos um número e uma letra e ter entre 8 a 15 caracteres.");
                    }
                }
